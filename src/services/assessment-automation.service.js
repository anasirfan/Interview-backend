...
Limi AI Recruitment Team`
  ...
  async sendExistingAssessmentEmail(candidateId, pdfFileName, emailData = null) {
    const candidates = await query('SELECT * FROM candidates WHERE id = ?', [candidateId]);
    if (!candidates || candidates.length === 0) {
      throw new Error('Candidate not found');
    }
    ...
    const candidate = candidates[0];
    const finalEmailData = emailData || await this.generateAssessmentEmail(candidate, {
      title: `Technical Assessment for ${candidate.position}`
    });

    await this.sendAssessmentEmail(candidateId, finalEmailData, pdfFileName);
